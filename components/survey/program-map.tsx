...
export function ProgramMapView({ map }: { map: ProgramMap }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl border border-white/10 bg-[#0d1b34]">
      ...
      >
        ...
        {map.links.map((l) => {
          ...
          return (
            <g key={`${l.a}|${l.b}`} opacity={opac}>
              ...
              <circle cx={(a.x + b.x) / 2} cy={(a.y + b.y) / 2} r={9} fill="#0d1b34" stroke={color} strokeWidth={1.5} />
        ...
        <g opacity={selected ? 0.35 : 1}>
          <circle cx={map.cx} cy={map.cy} r={map.sharedR} fill="rgba(255,255,255,0.03)" stroke="rgba(255,255,255,0.16)" strokeDasharray="4 5" />
          <text x={map.cx} y={map.cy - map.sharedR + 20} textAnchor="middle" fontSize={11} fontWeight={600} letterSpacing={1.2} fill="rgba(255,255,255,0.45)">
            SHARED PROGRAM
          </text>
          {map.shared.map((b) => (
        ...
        {map.clusters.map((c) => (
          ...
          >
            ...
            <text x={c.x} y={c.y - c.r - 12} textAnchor="middle" fontSize={15} fontWeight={700} fill="#fff">{c.name}</text>
            <text x={c.x} y={c.y - c.r + 5 - 12 + 16} textAnchor="middle" fontSize={10.5} fill="rgba(255,255,255,0.5)">
              {c.headcount} people
            </text>
            {c.bubbles.map((b) => (
      ...
      {/* Legend + hints */}
      <div className="pointer-events-none absolute bottom-3 left-3 flex flex-wrap items-center gap-x-4 gap-y-1 rounded-lg bg-[#0b1830]/85 px-3 py-2 text-[11px] text-white/60 backdrop-blur-sm">
        <span className="flex items-center gap-1.5"><span className="inline-block h-2.5 w-2.5 rounded-full border border-white/70 bg-transparent" /> office (named)</span>
        <span className="flex items-center gap-1.5"><Crown className="h-3 w-3 text-amber-300" /> leader</span>
        <span className="flex items-center gap-1.5"><span className="inline-block h-2.5 w-2.5 rounded-full bg-white/25" /> ×N seats</span>
        <span className="flex items-center gap-1.5"><span className="inline-block h-0.5 w-5 rounded bg-cyan-300" /> adjacency (ranked)</span>
        <span className="text-white/35">scroll to zoom · drag to pan · click a team to spotlight · double-click to reset</span>
      </div>
    </div>
  )
}
  ...
  if (b.kind === "office") {
    return (
      <g>
        ...
        {b.crowned && (
          <text x={x} y={y - (b.person ? 4 : -4)} textAnchor="middle" fontSize={11} fill="#fcd34d">♛</text>
        )}
        {b.person ? (
          <text x={x} y={y + 8} textAnchor="middle" fontSize={9} fontWeight={600} fill="#fff">
            {b.person.length > 14 ? b.person.slice(0, 13) + "…" : b.person}
          </text>
        ) : (
          <text x={x} y={y + 3.5} textAnchor="middle" fontSize={9} fill="rgba(255,255,255,0.75)">office</text>
        )}
      </g>
    )
  }
  const fill = b.kind === "collab" ? "rgba(255,255,255,0.10)" : b.kind === "flex" ? "rgba(255,255,255,0.05)" : "rgba(255,255,255,0.14)"
  return (
    <g>
      <title>{`${b.count ?? ""} ${b.sublabel ?? b.label} · ~${Math.round(b.sf).toLocaleString()} SF`}</title>
      <circle cx={x} cy={y} r={b.r} fill={fill} stroke="rgba(255,255,255,0.35)" strokeWidth={1} strokeDasharray={b.kind === "flex" ? "3 3" : undefined} />
      <text x={x} y={y + 1} textAnchor="middle" fontSize={Math.min(15, b.r * 0.5)} fontWeight={700} fill="#fff">{b.label}</text>
      {b.sublabel && (
        <text x={x} y={y + Math.min(15, b.r * 0.5) + 2} textAnchor="middle" fontSize={8.5} fill="rgba(255,255,255,0.6)">
...
function SharedBubble({ b, ox, oy }: { b: MapBubble; ox: number; oy: number }) {
  ...
  return (
    <g>
      ...
      <circle cx={x} cy={y} r={b.r} fill={isCollab ? "rgba(0,186,220,0.12)" : "rgba(255,255,255,0.07)"} stroke={isCollab ? "rgba(0,186,220,0.55)" : "rgba(255,255,255,0.3)"} strokeWidth={1.2} />
      <text x={x} y={y + 1} textAnchor="middle" fontSize={Math.min(13, b.r * 0.48)} fontWeight={700} fill="#fff">{b.label}</text>
      {b.sublabel && (
        <text x={x} y={y + Math.min(13, b.r * 0.48) + 2} textAnchor="middle" fontSize={8} fill="rgba(255,255,255,0.6)">
