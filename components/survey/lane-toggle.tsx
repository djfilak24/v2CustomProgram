...
export function LaneToggle({
  ...
  return (
    <div className="inline-flex items-center rounded-full border border-slate-200 bg-slate-100 p-1 dark:border-white/10 dark:bg-white/[0.04]">
  ...
  return (
    <button
      ...
      className={`flex items-center gap-1.5 rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${
        active ? "bg-[#00badc] text-slate-900" : "text-slate-500 hover:text-slate-900 dark:text-white/60 dark:hover:text-white"
      }`}
    >
      {icon}
      {children}
